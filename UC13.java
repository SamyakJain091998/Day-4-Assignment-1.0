...
public class UC13 implements I1{
	...
	private int computeWage(CompanyEmpWage companyEmpWage){
		...
		//public int dailyWageArray[] = new int[31];  
		//public final int dailyCount=0;
		...
		while(totalhours <= companyEmpWage.workingHoursPerMonth && workingdays <= companyEmpWage.workingDaysPerMonth){
			...
			totalhours+=emphrs;
			int dailyWageRecord = emphrs * companyEmpWage.wageph;
			dailyWageArray.add(dailyWageRecord);
			//dailyCount++;
			System.out.println("Employee worked for: "+emphrs+" hours today.");
		}
		totalwage = totalhours * (companyEmpWage.wageph);
		System.out.println("Daily wage records are as follows: " + dailyWageArray);
		return totalwage;
	}
